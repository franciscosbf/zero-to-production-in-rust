...
#[tokio::test]
async fn new_password_must_be_at_least_8_characters() {
    let app = spawn_app().await;

    let new_password = "a".repeat(7);
...
#[tokio::test]
async fn new_password_must_be_64_characters_max() {
    let app = spawn_app().await;

    let new_password = "a".repeat(65);
